    ...
    enum class Question(val question: String, val answers: List<String>) {
        ...
        NAME("Как меня зовут?", listOf("Бендер", "bender")) {
            ...
            override fun validate(answer: String): Pair<Boolean, String> =
                (answer.trim().firstOrNull()?.isUpperCase()
        ...
        MATERIAL("Из чего я сделан?", listOf("металл", "дерево", "metal", "iron", "wood")) {
            ...
            override fun validate(answer: String): Pair<Boolean, String> =
                answer.trim().contains("\\d".toRegex())
                    .not() to "Материал не должен содержать цифр\n"
        },
        BDAY("Когда меня создали?", listOf("2993")) {
            override fun nextQuestion(): Question = SERIAL
            override fun validate(answer: String): Pair<Boolean, String> =
                answer.trim().contains("[^\\d]".toRegex())
                    .not() to "Год моего рождения должен содержать только цифры\n"
        },
